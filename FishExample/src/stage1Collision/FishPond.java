...
public class FishPond extends GameGrid{
	...
	public FishPond() {
	    ...
	    setTitle("Fishpond - Stage 1 - Skeleton with CollisionListener");  
	    FishTrap trap = new FishTrap();
	    trap.addActorCollisionListener(trap);
	    addActor(trap, getRandomCenterLocation());
	    for (int i = 0; i < nrFish; i++) {
	    	Fish fish = new Fish(this, trap);
	    	trap.addCollisionActor(fish);
		    addActor(fish, getRandomEmptyLocation());
	    }
