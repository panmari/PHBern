...
{
  private static NxtRobot robot =  null;
   ...
   * @param robot the NxtRobot already connected
   * @param file the file to send
   */
  public static boolean sendFile(NxtRobot robot, File file)
  {
  ...
  {
    byte[] command = {SYSTEM_COMMAND_NOREPLY, OPEN_WRITE};
  ...
  private static void writeFile(byte[] data)
  {
    ...
    byte[] command = {SYSTEM_COMMAND_NOREPLY, WRITE, (byte)0};  // No handle
    System.arraycopy(command, 0, request, 0, command.length);
  ...
  {
    byte[] request = {SYSTEM_COMMAND_NOREPLY, CLOSE, (byte)0};  // No handle
    robot.sendData(request);
  }
