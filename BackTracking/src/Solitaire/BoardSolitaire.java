...
public class BoardSolitaire extends GameGrid implements GGMouseListener
{
  ...
  public boolean mouseEvent(GGMouse mouse)
  {
    ...
    Actor marble = getOneActorAt(location, Marble.class);
    Actor oneUp = getOneActorUp();

    //if there is a marble
    if(marble != null && isPossibleLocation(location))
    {
      //if no marble has been selected before, lift it up
      if(marble.getIdVisible() == DOWN && oneUp == null)
        marble.show(UP);
      //if one marble has been selected before and an other one is selected, set
      ...
      //if the selected marble has already been lifted up and selected again,
      //set it back down
      else
        marble.show(DOWN);
    }
    //if selected location is empty and a marble has been selected before check
    //if the marble between can be jumped and removed
    else if(oneUp != null && marble == null && isPossibleLocation(location))
    {
      Location upLoc = oneUp.getLocation();
      Double dir = upLoc.getDirectionTo(location);
      int distance = upLoc.getDistanceTo(location);

      if(distance == 2 || ((dir == 45 || dir == 135 || dir == 225 || dir == 315) && distance == 3))
      {
        Actor between = getOneActorAt(upLoc.getNeighbourLocation(dir));
