...
public class PokerTestSelf extends CardGame
{
  public enum Suit
  {
    KREUZ, KARO, HERZ, PIK
  ...
  private final Location middleLocation = new Location(300, 300);

  public PokerTestSelf()
  {
    super(600, 600);
    addStatusBar(30);
    ...
    hands[0].setTargetArea(new TargetArea(middleLocation));
    hands[1].setTargetArea(new TargetArea(middleLocation));
    ...
    CardAdapter cardAdapter = new CardAdapter()
    {
      ...
      {
    	Hand sourceHand = isInWhichHand(hands, card);
    	card.setVerso(true);
    	hands[2].setTargetArea(new TargetArea(sourceHand.getHandLocation()));
        card.transfer(hands[2], true, true); //to stack
        Card newCard = hands[2].getLast();
        newCard.setVerso(false);
        newCard.transfer(sourceHand, true, true);
      }
    };
   ...
   * @param hands
   * @param card
   * @return null, if not in any of @hands
   */
  private Hand isInWhichHand(Hand[] hands, Card card) {
	  for (Hand a: hands)
		  if (card.isInHand(a))
			  return a;
	  return null;
  ...
  {
    RowLayout rowLayout = new RowLayout(upperLocation, 350);
    hands[0].setView(this, rowLayout);
    hands[0].draw();
    RowLayout rowLayout1 = new RowLayout(bottomLocation, 350);
    hands[1].setView(this, rowLayout1);
    hands[1].draw();
    hands[2].setVerso(true);
    //doesn't work with stackLayout!
    hands[2].setView(this, new StackLayout(middleLocation));
    //hands[2].setView(this, new RowLayout(middleLocation, 500));
    hands[2].draw();
  }
  /**
