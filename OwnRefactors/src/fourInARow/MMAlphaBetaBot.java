...
public class MMAlphaBetaBot extends ComputerPlayer {

	private final int searchDepth = 2;
	private final int VALUE_QUAD = 1000, VALUE_TRIPPLE = 10, VALUE_PAIR = 2;
	private int solution;

	public MMAlphaBetaBot(ArrayManager am, int nbPlayer) {
		super(am, nbPlayer);
	}
	...
	private int alphabeta(int depth, int alpha, int beta, int player) {
		if (depth == 0) {
			debugInfo(am.getStringBoard(board));
			debugInfo("Value: " + evaluateSituation(player));
